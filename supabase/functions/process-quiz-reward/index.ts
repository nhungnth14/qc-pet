...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (petFetchErr || !pet)
      throw petFetchErr ?? new Error('Pet record not found');
    ...
    if (petUpdateErr || !updatedPet)
      throw petUpdateErr ?? new Error('Failed to update pet');
    ...
    if (barsFetchErr || !bars)
      throw barsFetchErr ?? new Error('need_bars record not found');
